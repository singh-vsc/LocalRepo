define(['modules/backbone-mozu', 'underscore', 'modules/jquery-mozu', 'modules/models-cart', 'modules/cart-monitor', 'hyprlivecontext', 'hyprlive', 'modules/preserve-element-through-render'], function (Backbone, _, $, CartModels, CartMonitor, HyprLiveContext, Hypr, preserveElement) {
    var CartView = Backbone.MozuView.extend({
        ...
        initialize: function () {
           ...
           // this.calculateTaxShipping(this.model);
        ...
        populateDropDowns: function() {
            var cart = this.model;
            if(localStorage.getItem('selectedShipping') && localStorage.getItem('selectedState')) {
               cart.set({'selectedShipping': localStorage.getItem('selectedShipping')});
               cart.set({'selectedState': localStorage.getItem('selectedState')}) ;
            }
            // this.render();

        },
        /*getStateValue: function(e) {
            var elm = e.target;
            var cart = this.model;
            var stateSelected = elm.options[elm.selectedIndex].value;
            if (localStorage) {
                    console.log("Selected State : "+stateSelected);
                    localStorage.setItem('stateName',stateSelected)
                    cart.set({'selectedState':stateSelected});
                  console.log("Local storage supported");
                }
        },*/
        populateShipping: function(){
                alert("Populate Shipping");
                // var elm = e.target;
                // var cartModel = CartModels.Cart.fromCurrent();
                var cart = this.model;
                var tax = 15;
                // console.log("Cart 112 : "+JSON.stringify(cartModel));
                // console.log("Total : "+JSON.stringify(cart));
                var tot = cart.get('shippingTotal');
                cart.set({'shippingTotal': 15});
                tot = cart.get('shippingTotal');
                var total = cart.get('discountedTotal');
                var newTotal = tot+total;
                ...
                cart.set({'selectedState': stateSel});
                cart.set({'selectedShipping': shippingSel});

                /*var shippingMethod = elm.options[elm.selectedIndex].value;
                ...
                // this.apiModel.prop(cart.toJSON());
                // this.model.sync('PUT', cart);
                // this.model.trigger('sync', cart);
                // cart.syncApiModel();
                // this.render();
